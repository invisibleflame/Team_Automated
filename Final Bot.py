...
ADAFRUIT_IO_KEY='aio_MSXw66ASir4ko54GJlldZQAat1Tl'
        ...
        if 'on' in recentmessage.lower():
            ...
            if 'fan' in recentmessage.lower():
                ...
                if fan == 1:
                    ...
                    bot.send_message(chat_id, ref)
                    fan = 1
                    fan-status=aio.feeds('fan-status')
                    aio.send_data(fan-status.key,'ON')    
                continue
            ...
            if 'light' in recentmessage.lower():
                ...
                if lights == 1:
                    ...
                    bot.send_message(chat_id, ref)
                    lights = 1
                    welcome-feed=aio.feeds('welcome-feed')
                    aio.send_data(welcome-feed.key,'ON')
                continue
            else:
